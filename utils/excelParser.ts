    ...
    return new Promise((resolve, reject) => {
        ...
        reader.onload = (e) => {
            try {
                ...
                // ⚡ Bolt Performance Optimization: Pre-compute header indices outside the loop
                // Avoids O(M*K) operations per row (where M is headers count, K is keywords count)
                // by resolving the column indices exactly once.
                ...
                dataRows.forEach((row, rowIndex) => {
                    // Get values by column index (O(1) lookup)
