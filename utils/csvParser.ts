...
const cleanAddress = (input: any): string => {
  ...
  // Attempt to extract content inside the second set of quotes which usually represents the label in Excel hyperlink formulas
  const match = raw.match(/"([^"]+)"\)$/);
  if (match && match[1]) {
    return match[1].trim();
  }
  
  const doubleQuoteMatch = raw.match(/""([^""]+)""\)$/);
