    ...
    return (
        <div className="p-6 max-w-4xl mx-auto h-full overflow-y-auto">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-8 min-h-[400px]">
                ...
                {/* Status Messages */}
                {procState.isActive && (
                    <div className={`mb-6 p-4 rounded-xl flex items-center gap-3 ${procState.status === 'error' ? 'bg-error-container text-on-error-container' : 'bg-primary-container text-on-primary-container'}`}>
                ...
                {activeTab === 'clients' && (
                    <div className="space-y-6 animate-fade-in">
                        <div className="bg-surface-container-high rounded-xl p-6">
                            <h3 className="font-bold text-primary mb-2">Instruções para Clientes</h3>
                        ...
                        <div className="grid md:grid-cols-2 gap-8 mt-8">
                            <div>
                                <label className="block text-sm font-medium text-on-surface mb-2">Vendedor Responsável (Destino)</label>
                                <div className="relative">
                                    <select
                                        className="w-full bg-surface-container-highest border-b border-outline rounded-t-lg p-3 text-sm text-on-surface focus:border-primary focus:bg-surface-container-highest outline-none transition-colors appearance-none"
                                ...
                                <p className="text-xs text-on-surface-variant mt-1">Os clientes importados serão atribuídos a este vendedor.</p>
                            </div>

                            <div className="flex items-end">
                                <label className={`flex flex-col items-center justify-center w-full h-32 border-2 border-dashed rounded-xl cursor-pointer transition-colors ${!targetUploadUserId ? 'border-outline-variant bg-surface-container opacity-50 cursor-not-allowed' : 'border-primary bg-primary-container/30 hover:bg-primary-container'}`}>
                                    <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                        <FileUp className={`w-8 h-8 mb-2 ${!targetUploadUserId ? 'text-outline' : 'text-primary'}`} />
                                        <p className={`text-sm font-medium ${!targetUploadUserId ? 'text-on-surface-variant' : 'text-on-surface'}`}>
                        ...
                        <div className="pt-6 border-t border-outline-variant mt-6">
                            <button
                                ...
                                className="text-error text-sm hover:underline flex items-center gap-1 font-medium"
                            >
                ...
                {activeTab === 'products' && (
                    <div className="space-y-6 animate-fade-in">
                        <div className="bg-tertiary-container rounded-xl p-6">
                            <h3 className="font-bold text-on-tertiary-container mb-2">Instruções para Produtos</h3>
                            <ul className="text-sm text-on-tertiary-container space-y-1 list-disc list-inside opacity-80">
                                <li>O arquivo deve ser formato <strong>.CSV</strong>.</li>
                        ...
                        <div className="mt-8">
                            <label className="flex flex-col items-center justify-center w-full h-48 border-2 border-tertiary border-dashed rounded-xl cursor-pointer bg-tertiary-container/30 hover:bg-tertiary-container/50 transition-colors">
                                <div className="flex flex-col items-center justify-center pt-5 pb-6">
                                    <Package className="w-10 h-10 text-tertiary mb-3" />
                                    <p className="mb-2 text-sm text-on-surface font-medium">Clique para carregar Catálogo de Produtos</p>
                                    <p className="text-xs text-on-surface-variant">Arquivo CSV (max 10MB)</p>
                                </div>
                                <input
                                    type="file"
                                    accept=".csv"
