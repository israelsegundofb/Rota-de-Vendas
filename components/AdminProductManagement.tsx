...
import { Package, Upload, Trash2, DollarSign, FileSpreadsheet, Search, Save, ArrowUp, ArrowDown, Sparkles, Loader2 } from 'lucide-react';
import { Product } from '../types';
import { parseProductCSV } from '../utils/csvParser';
import { processProductsWithAI } from '../services/geminiService';
...
const AdminProductManagement: React.FC<AdminProductManagementProps> = ({
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const newProducts = await parseProductCSV(file);
  ...
  const handleEnrichProducts = async () => {
    ...
    // Get API Key safely suitable (assuming stored in localStorage or env)
    ...
    if (!apiKey) {
      alert("Nenhuma chave de API configurada. Vá em Configurações > Chaves API para configurar.");
      return;
    }

    if (!confirm(`Deseja utilizar a IA para categorizar automaticamente ${products.length} produtos? Isso pode levar alguns minutos.`)) return;

    setIsEnriching(true);
    setEnrichmentProgress({ current: 0, total: products.length });

    try {
      const enriched = await processProductsWithAI(localProducts, apiKey, (current, total) => {
        setEnrichmentProgress({ current, total });
      });

      onUploadProducts(enriched); // Update parent state/App.tsx
      alert("Processamento concluído! As categorias foram atualizadas.");
    } catch (error) {
  ...
  // Filter first
  const filtered = localProducts.map((p, index) => ({ ...p, originalIndex: index })).filter(p =>
          ...
          <div className="flex gap-3">
            ...
            {products.length > 0 && (
              <button
                ...
                disabled={isEnriching || hasChanges}
                className={`px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-lg hover:from-purple-700 hover:to-indigo-700 text-sm font-bold flex items-center gap-2 transition-all shadow-md ${isEnriching ? 'opacity-80 cursor-not-allowed' : ''}`}
                title="Usar IA para categorizar produtos automaticamente"
              >
                {isEnriching ? (
                  <>
              ...
              <table className="w-full text-sm text-left">
                ...
                <tbody className="divide-y divide-gray-100 bg-white">
                  {displayedProducts.map((p) => (
                    <tr key={`${p.sku}-${(p as any).originalIndex}`} className="hover:bg-gray-50 group">
