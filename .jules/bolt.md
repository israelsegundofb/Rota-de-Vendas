...
## 2024-05-18 - Optimized Client Import performance via Hoisted Normalization
**Learning:** Normalizing strings repeatedly inside nested loops (`.findIndex`, `.forEach`) scaling identically causes massive garbage collection overhead.
**Action:** Pre-compute and hoist static property string normalizations for search iterations. Specifically, normalize the properties to find outside the inner loop iteration to avoid allocating the same string N times.
