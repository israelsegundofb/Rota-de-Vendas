...
## 2024-06-10 - React Render Loop Optimization with Map Lookups
**Learning:** In highly interactive components with complex derived state (like filtering active users or mapping messages/conversations), `Array.prototype.find()` operations within `map` and `filter` blocks scale poorly (`O(N*M)` complexity). When `M` (e.g., number of messages) and `N` (number of users) are large, this causes significant render blocking.
**Action:** Always pre-compute a lookup dictionary (`Map`) using `useMemo` for static or slowly-changing datasets (`allUsers`) before entering iteration blocks. Replace `allUsers.find(u => u.id === msg.senderId)` with `userMap.get(msg.senderId)`. This shifts the complexity to `O(N)` for initialization and `O(1)` for lookups, ensuring renders scale linearly with `M`. Also, ensure any search query normalization (e.g. `toLowerCase()`) is hoisted outside of loops, particularly inside `useMemo` blocks for derived lists like `filteredUsers`.
