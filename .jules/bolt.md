...
## 2024-05-26 - Hoisted String Normalization
**Learning:** Performing `trim().toLowerCase()` repeatedly on array objects inside an `O(N*M)` search loop causes massive string allocation overhead and performance lag during large CSV processing.
**Action:** When searching objects inside a loop using string transformation, hoist the transformation by creating an array of pre-normalized target objects outside the loop.
