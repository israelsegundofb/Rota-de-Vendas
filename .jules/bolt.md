...
## 2025-02-14 - [Replace chained array operations with single-pass loops for Set generation]
**Learning:** Chaining array methods like `.map().filter()` creates unnecessary intermediate array allocations, resulting in increased garbage collection (GC) overhead when processing large data collections.
**Action:** Replace chained array operations with single-pass `for` loops that directly populate a `Set` (or array) to avoid multiple memory allocations.
