  ...
  for (let i = 0; i < total; i += BATCH_SIZE) {
    ...
    while (!success && retries <= MAX_RETRIES) {
      try {
        ...
        // Fallback: If no Grounding URI, try to extract from CSV Hyperlink or generate search link
        if (!googleMapsUri) {
          // Try to extract URL from formula: =HYPERLINK("URL"; ...)
          const linkFormula = client['googleMapsLink'];
          if (linkFormula) {
            const urlMatch = linkFormula.match(/HYPERLINK\("([^"]+)"/);
            if (urlMatch && urlMatch[1]) {
              googleMapsUri = urlMatch[1];
            } else if (address) {
              // Generate search link if we have address but no link
