...
const App: React.FC = () => {
  ...
  const handleClientFileDirect = async (file: File, ownerId: string, skipConfirmation = false) => {
    ...
    try {
      ...
      // -----------------------------------------------------------------
      // Optimize O(N*M) lookup bottleneck: Hoist user normalization
      ...
      rawData.forEach(raw => {
        if (raw.salespersonName) {
          const rawName = raw.salespersonName.trim().toLowerCase();
          // Find exactly the user with that name (case insensitive)
          const match = normalizedUsers.find(u =>
            u.normName === rawName ||
            u.normUsername === rawName ||
            rawName.includes(u.normName) // Partial match (e.g. 'Israel França' in 'Israel França Silva')
          );
          if (match) {
      ...
      await processClientsSimple(
        ...
        (processed, total, latestClient?: EnrichedClient) => {
          ...
          if (latestClient) {
            ...
            // Pre-compute keys for optimization (avoid repetitive regex inside loop)
            const cleanNewCnpj = taggedNewClient.cnpj?.replace(/\D/g, '');
            const newNameNorm = taggedNewClient.companyName.toLowerCase().trim();
            ...
            setMasterClientList(prev => {
              ...
              const existingIdx = list.findIndex(c => {
                ...
                return c.companyName.toLowerCase().trim() === newNameNorm &&
