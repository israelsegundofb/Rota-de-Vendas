          ...
          try {
            ...
            if (activeCnpj && activeCnpj.length === 14) {
              ...
              if (fullData) {
                ...
                if (existingWithCnpj) {
                  // Optimization: Use a Set to deduplicate purchases in O(N) instead of O(N^2)
                  const allPurchases = [...(existingWithCnpj.purchasedProducts || []), ...(client.purchasedProducts || [])];
                  const seenKeys = new Set<string>();
                  const uniquePurchases = allPurchases.filter(p => {
                    const key = `${p.sku}|${p.purchaseDate}`;
                  ...
                  const mergedClient: EnrichedClient = {
                    ...
                    purchasedProducts: uniquePurchases
                  };
