...
export const useFilters = (
    ...
    const availableStates = useMemo(() => {
        // Optimize: Replace chained array operations (.filter().map().filter())
        // with a single-pass loop directly populating a Set. This prevents
        // multiple intermediate array allocations and reduces GC overhead.
        const states = new Set<string>();
        const filterRegionActive = filterRegion !== 'Todas';

        for (let i = 0; i < visibleClients.length; i++) {
            const c = visibleClients[i];
            if ((!filterRegionActive || c.region === filterRegion) && c.state) {
                states.add(c.state);
            }
        }
    ...
    const availableCities = useMemo(() => {
        // Optimize: Replace chained array operations (.filter().map().filter())
        // with a single-pass loop directly populating a Set.
        ...
        for (let i = 0; i < visibleClients.length; i++) {
            const c = visibleClients[i];
            if ((!filterRegionActive || c.region === filterRegion) && c.state === filterState && c.city) {
                cities.add(c.city);
            }
        }
    ...
    const productCategories = useMemo(() => {
        // Optimize: Replace chained array operations (.map().filter())
        // with a single-pass loop directly populating a Set.
    ...
    const productSections = useMemo(() => {
        ...
        const filterCatActive = filterProductCategory !== 'Todos';

        for (let i = 0; i < products.length; i++) {
            const p = products[i];
            if ((!filterCatActive || p.category === filterProductCategory) && p.section) {
                secs.add(p.section);
            }
        }
