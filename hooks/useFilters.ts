...
export const useFilters = (
    ...
    const filteredClients = useMemo(() => {
        ...
        // Optimize: Replace strings with pre-compiled RegExps for faster .test() checks in tight loops
        const query = debouncedSearchQuery || '';
        const prodQuery = debouncedProductQuery || '';

        // Escape special regex characters
        const escapedQuery = query.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        const searchRegex = escapedQuery ? new RegExp(escapedQuery, 'i') : null;

        const escapedProdQuery = prodQuery.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
        ...
        return visibleClients.filter(c => {
            ...
            if (filterProductCategory !== 'Todos' || filterProductSection !== 'Todas' || filterProductSku !== 'Todos' || prodRegex) {
