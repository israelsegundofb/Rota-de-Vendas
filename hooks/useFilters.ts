...
export const useFilters = (
    ...
    const filteredClients = useMemo(() => {
        // Optimize: Pre-parse filter dates to integers outside the loop to avoid creating Date objects
        const parsedStartInt = startDate ? parseDateToInt(startDate) : null;
        const parsedEndInt = endDate ? parseDateToInt(endDate) : null;
        const hasDateFilter = parsedStartInt !== null || parsedEndInt !== null;
                ...
                if (!c.purchasedProducts || c.purchasedProducts.length === 0) {
                    ...
                    for (let i = 0; i < c.purchasedProducts.length; i++) {
                        ...
                        if (!matchDate && p.purchaseDate) {
                            ...
                            if (pDateInt !== null) {
                                const isAfterStart = parsedStartInt === null || pDateInt >= parsedStartInt;
                                const isBeforeEnd = parsedEndInt === null || pDateInt <= parsedEndInt;
