        ...
        return visibleClients.filter(c => {
            ...
            if (filterProductCategory !== 'Todos' || filterProductSection !== 'Todas' || filterProductSku !== 'Todos' || prodQuery !== '') {
                ...
                if (!c.purchasedProducts || c.purchasedProducts.length === 0) {
                    ...
                    // Optimize: Combine 5 separate O(N) loops into a single O(N) loop over purchasedProducts
                    // while PRESERVING the original logic: "client matches if any of its products match Condition A,
                    // AND any of its products match Condition B, etc."
                    ...
                    for (const p of c.purchasedProducts) {
                        // Once all conditions are met by *any combination* of products, we can early exit!
